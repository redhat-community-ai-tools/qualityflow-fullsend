...
- Absolute: `<repo_path>/pkg/controllers/vm/vm.go`
- Relative: `pkg/controllers/vm/vm.go`
...
1. `incomingCalls` on HandleCPUHotplug → finds ReconcileResource
2. `outgoingCalls` on HandleCPUHotplug → finds ValidateCPUChange, UpdateSpec
3. `incomingCalls` on UpdateSpec → finds PrepareMigration (Level 2)
