#!/usr/bin/env bash
# pre-std-refiner.sh — Validate inputs before the STD refiner agent runs.
#
# STD refiner is file-only (reads STD + review locally).
#
# Required env vars:
...
echo "::notice::QualityFlow std-refiner: ${JIRA_TICKET:-unset}"
